...
TEST(ArcTest, ConstructionValid) {
    Arc arc(Point(0, 0), 5.0, 0.0, kPi / 2.0);
    EXPECT_DOUBLE_EQ(arc.radius(), 5.0);
    EXPECT_DOUBLE_EQ(arc.start_angle(), 0.0);
    EXPECT_DOUBLE_EQ(arc.end_angle(), kPi / 2.0);
}

TEST(ArcTest, ConstructionInvalidRadius) {
    EXPECT_THROW(Arc(Point(0, 0), 0.0, 0.0, kPi), std::invalid_argument);
    EXPECT_THROW(Arc(Point(0, 0), -1.0, 0.0, kPi), std::invalid_argument);
}

TEST(ArcTest, SweepAngle) {
    // Quarter circle
    Arc quarter(Point(0, 0), 1.0, 0.0, kPi / 2.0);
    EXPECT_NEAR(quarter.sweep_angle(), kPi / 2.0, 1e-10);

    // Half circle
    Arc half(Point(0, 0), 1.0, 0.0, kPi);
    EXPECT_NEAR(half.sweep_angle(), kPi, 1e-10);

    // Wrapping arc (from 3π/2 to π/2 = 270° sweep going through 0)
    Arc wrap(Point(0, 0), 1.0, 3.0 * kPi / 2.0, kPi / 2.0);
    EXPECT_NEAR(wrap.sweep_angle(), kPi, 1e-10);
}

TEST(ArcTest, Length) {
    Arc quarter(Point(0, 0), 2.0, 0.0, kPi / 2.0);
    EXPECT_NEAR(quarter.length(), 2.0 * kPi / 2.0, 1e-10);
}
...
TEST(ArcTest, Midpoint) {
    ...
    EXPECT_NEAR(mp.x(), std::cos(kPi / 4.0), 1e-10);
    EXPECT_NEAR(mp.y(), std::sin(kPi / 4.0), 1e-10);
}
...
TEST(ArcTest, ContainsAngle) {
    ...
    EXPECT_TRUE(arc.contains_angle(kPi / 4.0));
    EXPECT_TRUE(arc.contains_angle(kPi / 2.0));
    EXPECT_FALSE(arc.contains_angle(kPi));
    EXPECT_FALSE(arc.contains_angle(3.0 * kPi / 2.0));
}

TEST(ArcTest, ContainsAngleWrapping) {
    // Arc from 300° to 60° (wrapping through 0°)
    Arc arc(Point(0, 0), 1.0, 5.0 * kPi / 3.0, kPi / 3.0);
    EXPECT_TRUE(arc.contains_angle(0.0));
    EXPECT_TRUE(arc.contains_angle(kPi / 6.0));
    EXPECT_FALSE(arc.contains_angle(kPi));
}
...
TEST(ArcTest, ContainsPoint) {
    ...
    EXPECT_TRUE(arc.contains(Point(std::cos(kPi / 4.0), std::sin(kPi / 4.0))));
...
TEST(ArcTest, BoundingBoxFullCircle) {
    ...
    Arc arc(Point(0, 0), 1.0, 0.0, 3.0 * kPi / 2.0);
    BoundingBox bb = arc.bounding_box();
...
TEST(ArcTest, NegativeAngleNormalization) {
    Arc arc(Point(0, 0), 1.0, -kPi / 2.0, kPi / 2.0);
    // -π/2 normalizes to 3π/2
    EXPECT_NEAR(arc.start_angle(), 3.0 * kPi / 2.0, 1e-10);
    EXPECT_NEAR(arc.end_angle(), kPi / 2.0, 1e-10);
}
